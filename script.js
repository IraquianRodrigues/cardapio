...
// ao clicar em finalizar, abrir nosso modal do carrinho
...
// quando clicar na funcçao fechar, fechar o modal
closeModalBtn.addEventListener("click", function(){
    cartModal.style.display = "none" 
})

// adicionar nossos itens no carrinho
menu.addEventListener("click", function(event){

    let parantButton = event.target.closest(".add-to-cart-btn")

    // pegar o item que clicou
    if(parantButton){
...
// criando nossa mensagem para o whats
const message = encodeURIComponent(cartItems)
const phone = "84999740576" 

window.open(`https://wa.me/${phone}?text=${message} Endereço: ${addressInput.value}`, "_blank")

// zerar o carrinho depois de enviar o pedido pelo whats
cart = [];
updateCartModal();
})

// verificar se o restaurante está em horario de funcionamento
function checkRestaurantOpen(){
    const data = new Date()
    const hora = data.getHours()
    return hora >= 18 && hora < 2; //true
}
